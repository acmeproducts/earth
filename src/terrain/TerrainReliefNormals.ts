...
const maps = new WeakMap<AbstractMesh, { texture: RawTexture; width: number; depth: number }>();
...
export function attachTerrainReliefNormals(
  ...
  const texture = RawTexture.CreateRGBATexture(
    terrainReliefNormalPixels(terrain), terrain.width, terrain.height,
  ...
  maps.set(mesh, { texture, width, depth });
  mesh.onDisposeObservable.addOnce(() => { texture.dispose(); maps.delete(mesh); });
}
...
export class TerrainReliefNormalsPlugin extends MaterialPluginBase {
  ...
  prepareDefines(defines: MaterialDefines, _scene: Scene, mesh: AbstractMesh): void {
    (defines as MaterialDefines & { TERRAIN_RELIEF_NORMALS: boolean }).TERRAIN_RELIEF_NORMALS = maps.has(mesh);
  }
  ...
  hardBindForSubMesh(buffer: UniformBuffer, _scene: Scene, _engine: AbstractEngine, subMesh: SubMesh): void {
    const map = maps.get(subMesh.getMesh());
    if (!map) return;
...
#ifdef TERRAIN_RELIEF_NORMALS
vec3 reliefNormal = normalize(texture2D(terrainReliefNormals,
  vTerrainReliefXZ * terrainReliefUV.xy + terrainReliefUV.zw).rgb * 2.0 - 1.0);
