...
export function generateCloudDensityAtlasData(): CloudDensityAtlasData {
  ...
  for (let variant = 0; variant < CLOUD_VARIANT_COUNT; variant++) {
    const tile = renderCloudVariant(variant);
    writeAtlasTile(
      pixels,
      width,
      tile,
...
function renderCloudVariant(variant: number): Uint8Array {
  ...
  const upZ = -Math.sin(elevation);
  const rayY = -Math.sin(elevation);
  const rayZ = -Math.cos(elevation);
  ...
  for (let y = 0; y < CLOUD_TEXTURE_HEIGHT; y++) {
    ...
    for (let x = 0; x < CLOUD_TEXTURE_WIDTH; x++) {
      const imageX = ((x + 0.5) / CLOUD_TEXTURE_WIDTH * 2 - 1) * 1.18;
      let opticalDepth = 0;
      let peakDensity = 0;
      for (let step = 0; step < CLOUD_CAPTURE_STEPS; step++) {
        const ray = -1 + (step + 0.5) * rayStep;
        const sampleY = imageY * upY + ray * rayY;
        const sampleZ = imageY * upZ + ray * rayZ;
        const density = cloudDensity(lobes, imageX, sampleY, sampleZ, variant);
...
function bankLobes(variant: number): CloudLobe[] {
  ...
  const lobes: CloudLobe[] = [
    makeLobe(-0.34, -0.1 - slope, 0.02, 0.62, 0.22, 0.4, 0.9),
    makeLobe(0.4, -0.1 + slope, -0.04, 0.56, 0.2, 0.36, 0.88),
  ];
  for (let index = 0; index < 10; index++) {
    const x = lerp(-0.92, 0.92, lobeRandom(variant, index, 0));
    const centerBias = 1 - Math.min(1, Math.abs(x));
    lobes.push(makeLobe(
      x,
...
function clusteredLobes(variant: number): CloudLobe[] {
  ...
  for (let index = 0; index < 10; index++) {
    const x = anchorX + lerp(-0.72, 0.72, lobeRandom(variant, index, 0));
    const centerBias = 1 - Math.min(1, Math.abs(x - anchorX) / 0.72);
    lobes.push(makeLobe(
      x,
...
function brokenLobes(variant: number): CloudLobe[] {
  ...
  const lobes: CloudLobe[] = [
    makeLobe(-0.52, leftY, 0.02, 0.4, 0.25, 0.38, 0.94),
    makeLobe(0.53, rightY, -0.04, 0.36, 0.22, 0.34, 0.9),
  ];
  for (let index = 0; index < 10; index++) {
    const left = index < 6;
    const anchorX = left ? -0.54 : 0.55;
    const anchorY = left ? leftY : rightY;
    lobes.push(makeLobe(
      anchorX + lerp(-0.3, 0.3, lobeRandom(variant, index, 0)),
...
function towerLobes(variant: number): CloudLobe[] {
  const towerX = lerp(-0.3, 0.3, lobeRandom(variant, 0, 9));
  const lobes: CloudLobe[] = [
  ...
  for (let index = 0; index < 7; index++) {
    const upper = index >= 4;
    lobes.push(makeLobe(
      (upper ? towerX : 0) + lerp(-0.5, 0.5, lobeRandom(variant, index, 0)),
...
function writeAtlasTile(
  ...
  for (
    ...
    y < CLOUD_TEXTURE_HEIGHT + CLOUD_TEXTURE_GUTTER;
    y++
  ) {
    const sourceY = Math.max(0, Math.min(CLOUD_TEXTURE_HEIGHT - 1, y));
    for (let x = -CLOUD_TEXTURE_GUTTER; x < CLOUD_TEXTURE_WIDTH + CLOUD_TEXTURE_GUTTER; x++) {
      const sourceX = Math.max(0, Math.min(CLOUD_TEXTURE_WIDTH - 1, x));
      const source = (sourceY * CLOUD_TEXTURE_WIDTH + sourceX) * 4;
      const target = (
